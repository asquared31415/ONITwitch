...
internal static class SurpriseBoxPatches {
    ...
    private static class EnabledEntityPatches {
        ...
        private static IEnumerable<MethodBase> TargetMethods() {
            ...
            var types = new[] {
                // base game comets
                typeof(DustCometConfig), typeof(GoldCometConfig), typeof(IronCometConfig), typeof(RockCometConfig),
                typeof(CopperCometConfig),

                // DLC comets
                // note: The actual prefab is not created if DLC is not enabled, so it can't be chosen
                typeof(NuclearWasteCometConfig), typeof(AlgaeCometConfig), typeof(BleachStoneCometConfig),
                typeof(FullereneCometConfig), typeof(OxyliteCometConfig), typeof(PhosphoricCometConfig),
                typeof(SlimeCometConfig), typeof(SnowballCometConfig), typeof(UraniumCometConfig),

                // extra comets
                typeof(FoodCometConfig), typeof(GassyMooCometConfig),
            };
    ...
    // elements that are too hot should also be excluded because they spawn inside the base without insulation
        ...
        public static void Postfix() {
            foreach (var go in Assets.Prefabs) {
                if (go.TryGetComponent(out PrimaryElement primaryElement) &&
                    (primaryElement.Element.disabled ||
                     primaryElement.ElementID == TwitchSimHashes.OniTwitchIndestructibleElement ||
                     primaryElement.Element.defaultValues.temperature >
                     GameUtil.GetTemperatureConvertedToKelvin(85, GameUtil.TemperatureUnit.Celsius))) {
